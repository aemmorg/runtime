...
class SecretsGenerator:
    ...
    def run(cls, mode: str) -> None:
        ...
        provider = SecretsSettings.instance().get_secrets_provider_or_none()
        if provider is None:
            raise RuntimeError("")
        if mode == "recreate":
            os.remove(str(provider._get_secrets_dir() / ".secrets.yaml"))
...
if __name__ == "__main__":
    ...
    def parse_args():
        parser = argparse.ArgumentParser(description="Run text generation with specified model and prompt.")
