...
class Db(DbKey, RecordMixin, ABC):
    ...
    def load_many(
        ...
        *,
        ...
        Args:
            ...
            datasets: Sequence of backslash-delimited dataset identifiers (None means all datasets)
    ...
    def _check_datasets(cls, datasets: Sequence[str] | None) -> None:
        """Error if datasets has invalid format. None is valid and means all datasets."""
        if datasets is None:
            return
        if not isinstance(datasets, Sequence) or isinstance(datasets, str):
