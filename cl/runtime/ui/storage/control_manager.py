...
class ControlManager:
    ...
    def load(self) -> list[Control]:
        ...
        return ControlLoader(root_node=self.root_node.get_key().build()).load()

    def load_one(self) -> Control:
        """Load the root control from the database by the given root node."""
        return active(DataSource).load_one(self.root_node.get_key().build())
    ...
    def reset(self, **kwargs):
        ...
        """
        previous_controls = ControlLoader(root_node=self.root_node.get_key().build()).load()
