...
class EnvSettings(Settings):
    ...
    """Determines the default settings for multiuser access and data retention (set to TEST by pytest)."""
    ...
    def __init(self) -> None:
        ...
        # Convert env_kind from PascalCase string to enum
        if isinstance(self.env_kind, str):
            # From case-insensitive string to PascalCase
            env_kind_pascal_case = CaseUtil.snake_to_pascal_case(self.env_kind.lower())
        ...
        # Check env_user for safety before substitution
        ...
        if self.env_tenant is None:
            ...
            # Check env_tenant template for safety before substitution
        ...
        # Check env_tenant for safety again after the substitution
        IdentifierUtil.guard_valid_identifier(self.env_tenant)

        if self.env_dir is None:
            # Default if not specified via Dynaconf
            self.env_dir = ResourcesUtil.get_resources_root()
        else:
            # Check env_dir for safety before substitution
